...
from python.eHRL.environment.Edge_MovieLensEnvironment import EHRLMovieLensEnvironment
from python.eHRL.algorithm.eHRL_algorithm import eHRLAlg
from python.eHRL.agent.eHRL_edge_agent import eHRLEdgeAgent
from python.eHRL.agent.eHRL_cloud_agent import eHRLCloudAgent

class EHRLMain(object):
    ...
    def __init__(self, args):
       ...
       # self.train_outputPath = os.path.join(self.output_dir, append_sample_suffix(args.train_output, self.sample))
        ...
        self.env = EHRLMovieLensEnvironment(self.hyperparams.ENVconfig)
        ...
        self.agent_edge = eHRLEdgeAgent(self.hyperparams.AGEconfig)
        self.agent_edge.init(self.env, self.hyperparams.ALGconfig['batch_size'])

        self.agent_cloud = eHRLCloudAgent(self.hyperparams.AGEconfig)
        self.agent_cloud.init(self.env, self.hyperparams.ALGconfig['batch_size'])

        ## init algorithm module
        self.alg = eHRLAlg(self.hyperparams.ALGconfig)
        self.alg.initEnv(self.env)
        self.alg.initAgent(self.agent_cloud, self.agent_edge)
    ...
    def offline_test(self):
        # # Testing
        ratings, unknown, random_seen = self.alg.test_actor(self.env.train_users_data, target=False, n_round=3, sess=self.sess)
        ...
        ratings, unknown, random_seen = self.alg.test_actor(self.env.train_users_data, target=True, n_round=3, sess=self.sess)
        print('%0.1f%% unknown' % (100 * unknown / (len(ratings) + unknown)))
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run the eHRL algorithm.')
    ...
    ehrl_main = EHRLMain(args)

    #if args.mode == "train":
    ehrl_main.offline_learn()
    #if args.mode == "predict":
    ehrl_main.offline_test()
