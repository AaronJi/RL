...
class eHRLAlg(object):
    ...
    def __init__(self, hyperparams):
        ...
        self.agent_cloud = None
        self.agent_edge = None
        self.env = None
        return

    def initAgent(self, agent_cloud, agent_edge):
        self.agent_cloud = agent_cloud
        self.agent_edge = agent_edge
    ...
    def train(self, sess, summary_ops, summary_vars, writer):
        ...
        #self.agent_cloud.init_target_network(sess)
        self.agent_edge.init_target_network(sess)
        ...
        for i_session in range(self._hyperparams['n_session']):  # '4: for session = 1, M do'
            ...
            for t in range(self._hyperparams['n_session_len']):  # '7: for t = 1, T do'
                ...
                action = self.agent_edge.actor.act(state, sess=sess)
    ...
    def test_actor(self, users_data, target=False, n_round=1, sess=None):
        ...
        for i in range(n_round):
            ...
            for user_data in users_data:
                ...
                rec_items = self.agent_edge.get_recommendation_items(state, self.env.dict_embeddings, target=target, sess=sess)
