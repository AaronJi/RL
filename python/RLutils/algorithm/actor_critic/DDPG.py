...
def experience_replay(states, actions, rewards, next_states, actor, critic, gamma, recurrent_length=None, sess=None):
    '''
    Experience replay.
    Args:
    ...
    recurrent_tensor_size = [recurrent_length] * batch_size
...
def experience_replay_mc(states, actions, rewards, next_states, states_cloud, next_states_cloud, actor, critic, gamma, recurrent_length=None, sess=None):
    '''
    Experience replay WITH cloud-mobile collaboration.
    Args:
      experience: [states, actions, rewards, next_states].
      batch_size: sample size.
      actor: Actor network.
    ...
    # Calculate predicted Q′(s′, a′|θ^µ′) value
    target_Q_value = critic.predict(sess, next_states_cloud, next_actions, recurrent_tensor_size, target=True)

    # '24: Set y = r + γQ′(s′, a′|θ^µ′)'
    expected_rewards = rewards + gamma * target_Q_value

    # '25: Update Critic by minimizing (y − Q(s, a|θ^µ))²'
    Q_value, loss_critic, _ = critic.train(sess, states_cloud, actions, recurrent_tensor_size, expected_rewards)

    # '26: Update the Actor using the sampled policy gradient'
    action_gradients = critic.get_action_gradients(sess, states_cloud, next_actions, recurrent_tensor_size)
    actor.train(sess, states, recurrent_tensor_size, action_gradients)
